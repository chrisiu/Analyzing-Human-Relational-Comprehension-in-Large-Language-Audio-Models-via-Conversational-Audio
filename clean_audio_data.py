...
df = pd.read_csv('audiofiles_transcripts.csv')

df = df[~df['mixed_audio_filename'].isin(corrupted_files)]

df = df[(df['transcript_a'].notna() & (df['transcript_a'] != '')) & 
        (df['transcript_b'].notna() & (df['transcript_b'] != ''))]

df["relationship_detail_a"] = df["relationship_detail_a"].str.strip()
df["relationship_detail_b"] = df["relationship_detail_b"].str.strip()
df["label"] = df["label"].str.strip()

mask_fam = (df["label"] == "naturalistic") & (
    df["relationship_detail_a"].isin(["parent_child", "siblings"]) | 
    df["relationship_detail_b"].isin(["parent_child", "siblings"])
)
df.loc[mask_fam, ["relationship_detail_a", "relationship_detail_b"]] = "family-members"
df["relationship_detail_a"] = df["relationship_detail_a"].replace("family-generic", "family-members")
df["relationship_detail_b"] = df["relationship_detail_b"].replace("family-generic", "family-members")

mask_acting = (df["label"] == "improvised") & (
    (df["relationship_detail_a"] == "stranger") | (df["relationship_detail_b"] == "stranger")
)
df.loc[mask_acting, ["relationship_detail_a", "relationship_detail_b"]] = "stranger-acting"

df = df[~df["relationship_detail_a"].isin(["familiar-generic"])]

df.to_csv('audiofiles_transcripts.csv', index=False)
